...
잠깐만요, 뭔가 이상한 부분을 눈치채셨나요? 두번째 줄에서 `t[0] += [6, 7]`을 실행했을 때 TypeError가 나타났는데 다음 줄에서 `t`를 출력해보니 `t[0]`에 `[6, 7]`이 정상적으로 합쳐져있습니다. 일단 list와 list 간 `+=` 연산자를 상용하여도 새로운 리스트가 생성되는 것이 아니라 기존의 리스트에 더할 리스트가 합쳐지는 것이므로 `t[0] += [6, 7]`은 당연히 정상적으로 실행되어야 하는 것이 맞습니다. 네번째 줄의 `t[0] = [6, 7, 8]`는 아예 새로운 리스트로 교체하는 것이므로 불가한 것이 맞습니다.

이러한 현상이 일어나는 이유는 다음 포스트에 작성해보록 하겠습니다.
<br>
<br>
<br>
